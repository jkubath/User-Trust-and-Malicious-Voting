		...
		for user in malicious_ids:
			...
			if business in malicious_votes:
				if user in malicious_votes[business]:
					...
					malicious_votes[business].append(user)
			else:
				malicious_votes[business] = [user]

			if VERBOSE >= 2:
				print("\tHonest vote {} - {} {:.6f}".format(user, business, remaining))
...
def RepTrap(folder, targetUsers, malicious_ids, target):
	...
	total_votes = 0 # malicious user votes count
	...
	while True:
		...
		if all_business[target].stars >= 3 and malicious_trust > target_trust:
			...
			if votes > 0:
				total_votes += votes
				print("--Success votes: {}".format(total_votes))
				return # exit
		...
		# UpdateSimulation
		if remaining < 0:
			print("Remaining {:.3f} {}".format(remaining, total_votes))
			continue
		else:
